# swap_case.asm program
# For CMPSC 64
#
# Data Area
.data
    buffer:         .space 100
    input_prompt:   .asciiz "Enter string:\n"
    output_prompt:  .asciiz "Output:\n"
    convention:     .asciiz "Convention Check\n"
    newline:        .asciiz "\n"

.text

#
# DO NOT MODIFY THE MAIN PROGRAM 
#       OR ANY OF THE CODE BELOW, WITH 1 EXCEPTION!!!
# YOU SHOULD ONLY MODIFY THE SwapCase FUNCTION 
#       AT THE BOTTOM OF THIS CODE
#
main:
    la $a0, input_prompt    # prompt user for string input
    li $v0, 4
    syscall

    li $v0, 8       # take in input
    la $a0, buffer
    li $a1, 100
    syscall
    move $s0, $a0   # save string to s0

    li $s1, 0
    li $s2, 0
    li $s3, 0
    li $s4, 0
    li $s5, 0
    li $s6, 0
    li $s7, 0

    move $a0, $s0
    jal SwapCase

    add $s1, $s1, $s2
    add $s1, $s1, $s3
    add $s1, $s1, $s4
    add $s1, $s1, $s5
    add $s1, $s1, $s6
    add $s1, $s1, $s7
    add $s0, $s0, $s1

    la $a0, output_prompt    # give Output prompt
    li $v0, 4
    syscall

    move $a0, $s0
    jal DispString

    j Exit

DispString:
    addi $a0, $a0, 0
    li $v0, 4
    syscall
    jr $ra

ConventionCheck:
    addi    $t0, $zero, -1
    addi    $t1, $zero, -1
    addi    $t2, $zero, -1
    addi    $t3, $zero, -1
    addi    $t4, $zero, -1
    addi    $t5, $zero, -1
    addi    $t6, $zero, -1
    addi    $t7, $zero, -1
    ori     $v0, $zero, 4
    la      $a0, convention
    syscall
    addi    $v0, $zero, -1
    addi    $v1, $zero, -1
    addi    $a0, $zero, -1
    addi    $a1, $zero, -1
    addi    $a2, $zero, -1
    addi    $a3, $zero, -1
    addi    $k0, $zero, -1
    addi    $k1, $zero, -1
    jr      $ra
    
Exit:
    li $v0, 10
    syscall

# COPYFROMHERE - DO NOT REMOVE THIS LINE

# YOU CAN ONLY MODIFY THIS FILE FROM THIS POINT ONWARDS:
SwapCase:
    #TODO: write your code here, $a0 stores the address of the string
    
    # Do not remove the "jr $ra" line below!!!
    # It should be the last line in your function code!
#Take the string whose address is in $a0 and modify each character 
#in memory so uppercase ↔ lowercase.


    addi $sp, $sp, -12 # push 3 
    sw $ra, 8($sp)
    sw $s0, 4($sp)
    sw $s1, 0($sp)



    move $s0, $a0 #s0 hold teh address of each byte 


loop:
    lbu $s1, 0($s0)  #t1 holds teh actual value 
    beq $s1, $zero, done

    #if statment that checks upper
    li $t0, 65    # A
    li $t1, 90    # Z

    #check if in between these numbers 
    blt $s1, $t0, lower
    bgt $s1, $t1, lower

    # print original
    move $a0, $s1
    li $v0, 11
    syscall

    # newline
    li $a0, 10
    li $v0, 11
    syscall

    # Convert to lowercase
    addi $s1, $s1, 32

    # print new letter 
    move $a0, $s1
    li $v0, 11
    syscall

    # newline
    li $a0, 10
    li $v0, 11
    syscall


    sb $s1, 0($s0) #save it 

    jal ConventionCheck #prompt 
    j next


lower:

    li $t0, 97     # a
    li $t1, 122     #z

    blt $s1, $t0, next 
    bgt $s1, $t1, next 

    # Print original
    move $a0, $s1
    li $v0, 11
    syscall

    li $a0, 10
    li $v0, 11
    syscall

    # is lowercase
    addi $s1, $s1, -32     # convert to uppercase

    # Print changed 
    move $a0, $s1
    li $v0, 11
    syscall

    li $a0, 10
    li $v0, 11
    syscall

    sb $s1, 0($s0)         # store

    jal ConventionCheck


next:
    # Move pointer to next in string
    addi $s0, $s0, 1

    #repeat loop
    j loop


done:
    lw $s1, 0($sp)
    lw $s0, 4($sp)
    lw $ra, 8($sp)
    addi $sp, $sp, 12
    jr $ra




